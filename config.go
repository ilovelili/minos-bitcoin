...
const (
	defaultConfigFilename        = "minos.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "minos.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-minos.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = btcutil.AppDataDir("minos", false)
...
// config defines the configuration options for minos.
//
...
type config struct {
	...
	AgentBlacklist       []string      `long:"agentblacklist" description:"A comma separated list of user-agent substrings which will cause minos to reject any peers whose user-agent contains any of the blacklisted substrings."`
	AgentWhitelist       []string      `long:"agentwhitelist" description:"A comma separated list of user-agent substrings which will cause minos to require all peers' user-agents to contain one of the whitelisted substrings. The blacklist is applied before the blacklist, and an empty whitelist will allow all agents that do not fail the blacklist."`
...
//
// The above results in minos functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		minosLog.Infof("RPC service is disabled")
	}
	...
	if configFileError != nil {
		minosLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-minos.conf to the given destination path,
...
// minosDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func minosDial(addr net.Addr) (net.Conn, error) {
...
// minosLookup resolves the IP of the given host using the correct DNS lookup
...
func minosLookup(host string) ([]net.IP, error) {
